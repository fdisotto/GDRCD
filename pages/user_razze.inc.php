    ...
    <?php else: ?>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <?php while ($row = gdrcd_query($result, 'fetch')):
                ...
                // Versione in-game: usa immagine come icon e immagine grande
                $icon_src = !empty($row['immagine']) ? $img_dir . htmlspecialchars($row['immagine']) : null;
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                <?php for ($i = 0; $i < 6; $i++):
                                    ...
                                    if ($val > 0)      $cls = 'gdrcd-badge-success';
                                    elseif ($val < 0)  $cls = 'gdrcd-badge-error';
                                    else               $cls = 'gdrcd-badge-neutral';
                                    $sign = $val > 0 ? '+' : '';
                                    ?>
                                    <span class="<?= $cls ?>">
                                        <?= gdrcd_filter('out', $PARAMETERS['names']['stats']['car'.$i]) ?>
                                        <strong class="ml-0.5"><?= $sign . $val ?></strong>
                                    </span>
                                <?php endfor; ?>
                            </div>
                        </div>
