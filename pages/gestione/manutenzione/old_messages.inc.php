...
gdrcd_query("DELETE FROM messaggi    WHERE DATE_SUB(NOW(), INTERVAL " . $m . " MONTH) > spedito");
gdrcd_query("OPTIMIZE TABLE messaggi");
gdrcd_query("DELETE FROM backmessaggi WHERE DATE_SUB(NOW(), INTERVAL " . $m . " MONTH) > spedito");
gdrcd_query("OPTIMIZE TABLE backmessaggi");
?>
<div class="gdrcd-alert-success">
    <svg class="w-5 h-5 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
    <div>
        <strong>Messaggi vecchi rimossi</strong> (più vecchi di <?= (int)$m ?> mesi).
