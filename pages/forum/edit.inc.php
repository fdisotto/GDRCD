...
$id_msg = gdrcd_filter('num', $_POST['id_messaggio'] ?? 0);
$row = gdrcd_query(
    "SELECT autore, titolo, messaggio, id_messaggio_padre
     FROM messaggioaraldo WHERE id_messaggio = " . $id_msg
);
...
if ($can_edit) {
    ...
    gdrcd_query(
        "UPDATE messaggioaraldo SET
            ...
            titolo = '" . gdrcd_filter('in', $_POST['titolo'] ?? $row['titolo']) . "'
         WHERE id_messaggio = " . $id_msg . " LIMIT 1"
    );

    $padre = ((int)$row['id_messaggio_padre'] === -1) ? $id_msg : (int)$row['id_messaggio_padre'];
    gdrcd_redirect('main.php?page=forum&op=read&what=' . $padre . '&where=' . gdrcd_filter('num', $_POST['araldo'] ?? 0));
} else {
    ?>
