...
class Engine
{
     ...
     * @return Migration[] Un array con un oggetto già istanziato per ogni migrazione, nel corretto ordine di
    ...
    private static function loadMigrationClasses(){
        ...
        foreach(new DirectoryIterator(self::MIGRATIONS_FOLDER) as $fileInfo){
            ...
            // Le migration nel filesystem usano ancora il global namespace
            // (class XxxMigration extends DbMigration), quindi le risolviamo
            // come classi globali (\ClassName).
            if(class_exists($className)){
                $reflected = new ReflectionClass($className);
                // Accetta sia la classe base storica (DbMigration) sia, in
                // futuro, eventuali migration nel namespace GDRCD\DbMigration.
                if($reflected->isSubclassOf("DbMigration") || $reflected->isSubclassOf(Migration::class)){
